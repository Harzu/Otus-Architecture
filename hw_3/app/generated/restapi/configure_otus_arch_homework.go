...
func configureAPI(api *operations.OtusArchHomeworkAPI) http.Handler {
	...
	if api.UserListHandler == nil {
		api.UserListHandler = operations.UserListHandlerFunc(func(params operations.UserListParams) middleware.Responder {
			return middleware.NotImplemented("operation .UserList has not yet been implemented")
		})
	}
...
func setupCustomRoutes(next http.Handler) http.Handler {
	...
	router.Handle(constants.HealthCheckURL, http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if !srv.PostgresClient.HealthCheck() {
			http.Error(w, "psql not connected", http.StatusInternalServerError)
			return
		}

		data := &entities.HealthCheck{Status: "OK"}
